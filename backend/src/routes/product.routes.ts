...
import { CreateProductSchema, type CreateProductDTO } from "../validators/product.validator.js";
...
const handleCreateProduct: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const addedByUid = req.user!.userId;
        const newProduct = await ProductService.createProduct(req.body as CreateProductDTO, addedByUid);
        res.status(201).json({
            success: true,
            data: newProduct,
        })
